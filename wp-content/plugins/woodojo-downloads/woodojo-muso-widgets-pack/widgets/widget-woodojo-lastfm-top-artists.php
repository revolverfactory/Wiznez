...
/**
 * WooDojo Last.fm Top Artists Widget
 *
 * A WooDojo Last.fm top artists widget.
 *
 * @package WordPress
 * @subpackage WooDojo
 ...
 */
class WooDojo_Widget_Lastfm_Top_Artists extends WP_Widget {
	...
	public function __construct () {
		...
		$this->woo_widget_cssclass = 'widget_woodojo_lastfm_top_artists';
		$this->woo_widget_description = __( 'Your top Last.fm artists on your site', 'woodojo-muso-widgets-pack' );
		$this->woo_widget_idbase = 'woodojo_lastfm_top_artists';
		$this->woo_widget_title = __( 'Last.fm - Top Artists', 'woodojo-muso-widgets-pack' );
		
		$this->api_key = '055930be28ded505a670a7e998c5b1d7';
		$this->transient_expire_time = (int) apply_filters( 'woodojo_lastfm_top_artists_cache', 60 * 60 * 24 ); // 1 Day
	...
	public function widget( $args, $instance ) {
		...
		$data = $this->request( array( 'user.getTopArtists' ), array( 'user' => $instance['username'], 'limit' => $limit, 'period' => $instance['period'] ) );
		
		foreach( $data->topartists as $artists ) {
			if( is_array( $artists ) && $limit > 0 ) {
				$html .= '<ol id="woodojo_lastfm_top_list">';
				for($i = 1; $i <= $limit; $i++) { 
					if( isset( $artists[$i] ) ) {
						$html .= '<li class="woodojo_lastfm_top_' . $i . '">';
							if( $instance['link'] == 1 ) {
								$html .= '<a href="'.$artists[$i]->url.'">';
							}
								if( $instance['include_thumb'] == 1 ) {
									$image = (array) $artists[$i]->image[1];
									if ( isset( $image['#text'] ) && ( $image['#text'] == '' ) ) {
										$image['#text'] = $woodojo_muso_widgets_pack->plugin_url . 'assets/images/default.png';
									}
									$html .= '<img class="woodojo_lastfm_thumb" src="' . $image['#text'] . '" />';
								}
								$html .= '<span class="artist">' . $artists[$i]->name  . '</span>';
								if( $instance['include_playcount'] == 1 ) {
									$html .= ' <span class="playcount">(' . $artists[$i]->playcount . ')</span>';
								}
							if( $instance['link'] == 1 ) {
								$html .= '</a>';
							}
	...
	public function update ( $new_instance, $old_instance ) {
		...
		if( $old_instance['limit'] != $new_instance['limit'] ) {
			delete_transient( 'woodojo_lastfm_user.getTopArtists_' . $instance['username'] );
		}
   ...
   public function form ( $instance ) {
		...
		$defaults = array(
			'title' => __( 'Last.fm Top Artists', 'woodojo-muso-widgets-pack' ),
			'username' => '',
			'link' => 1,
			'include_playcount' => 1,
			'include_thumb' => 1,
...
?>
		...
		<!-- Number of Artists: Text Input -->
		<p>
			<label for="<?php echo $this->get_field_id( 'limit' ); ?>"><?php _e( 'Number of Artists:', 'woodojo-muso-widgets-pack' ); ?></label>
...
<?php		
	...
	private function get_checkbox_settings () {
		return array(
			'link' => __( 'Link Artists', 'woodojo-muso-widgets-pack' ),
