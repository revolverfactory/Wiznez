...
class WooDojo_SoundCloud {
	 ...
	 * @return void
	 */
	public function __construct ( $username, $settings = array() ) {
		$this->plugin_path = '';
		$this->plugin_url = '';
	...
	public function init() {
		...
		add_action( 'wp_ajax_woodojo_soundcloud_refresh', array( &$this, 'refresh_transients' ) );
		add_action( 'wp_ajax_nopriv_woodojo_soundcloud_refresh', array( &$this, 'refresh_transients' ) );
		
		if ( is_admin() ) {
			global $pagenow;

			if ( isset( $_GET['page'] ) && ( $_GET['page'] == 'woodojo-muso-widgets-pack' ) || $pagenow == 'admin.php' ) {
	...
	public function get_tracks () {
		$transient_key = 'woodojo_' . $this->token . '_tracks';
	...
	public function get_playlists () {
		$transient_key = 'woodojo_' . $this->token . '_playlists';
	 ...
	 */
	public function generate_player ( $id, $format = 'tracks', $args = array() ) {
		$defaults = array(
			'type' => 'standard', 
			'width' => '100%', 
			'height' => '300', 
	...
	public function enqueue_scripts () {
		wp_register_script( 'woodojo-' . $this->token . '-admininterface', $this->plugin_url . 'assets/js/settings.js', '', '1.0.0' );
		wp_enqueue_script( 'woodojo-' . $this->token . '-admininterface' );
		...
		wp_localize_script( 'woodojo-' . $this->token . '-admininterface', 'woo_localized_data', $data );
	} // End enqueue_scripts()
	 ...
	 * @return string $data
	 */
	public function api_request ( $params = array(), $endpoint = '.json' ) {
		$data = '';
	...
	public function refresh_transients () {
		...
		delete_transient( 'woodojo_' . $this->token . '_tracks' );
		delete_transient( 'woodojo_' . $this->token . '_playlists' );
