<?php
class Ci_redisException extends Exception {}
/**
...
class CI_redis {
    ...
    function __construct($host='10.1.1.113', $port = 6379) {
        $this->host = $host;
        $this->port = $port;
    }
    ...
    {
        $random = rand(0, $this->llen($list) - 1);
        return $this->lindex($list, $random);
    }

    function randomListItems($list, $count)
    {
        $x          = 0;
        $return     = array();
        $listLength = $this->llen($list);

        while($x < $count)
        {
            $x++;
            $random     = rand(0, $listLength - 1);
            $return[]   = $this->lindex($list, $random);
        }
        return $return;
    }

    function doesValueExistInList($value, $list)
    {   $exists = FALSE;
        $index  = $this->lindex($list, $value);
        $exists = ($index ? TRUE : FALSE);
        if($exists) return TRUE;

        foreach($this->lrange($list, 0, -1) as $item)
            if($item == $value)
                $exists = TRUE;
    ...
    function trackLoadingTime($identifier)
    {
        global $currentAppRunTime, $appRunTimeId;

        $currentTime    = microtime(true);
        $timeSoFar      = ($currentTime - $currentAppRunTime);

        return $this->CI->db->query("INSERT INTO runTimeTimeTracker (scriptId, identifier, currentTime, timeSoFar) VALUES ('$appRunTimeId', '$identifier', '$currentTime', '$timeSoFar')");
    }
    ...
    private function connect() {
        ...
        throw new Ci_redisException ( "$msg." );
    }
    ...
    private function read() {
        ...
        $this->disconnect ();
        throw new Ci_redisException ( "Cannot read from socket." );
    }
    private function cmdResponse() {
        ...
        switch ($s [0]) {
            case '-' : // Error message
                throw new Ci_redisException ( substr ( $s, 1 ) );
                break;
            ...
            case '*' : // Multi-bulk data (a list of values)
                ...
                break;
            default :
                throw new Ci_redisException ( 'Unknown responce line: ' . $s );
                break;
        }
    }
