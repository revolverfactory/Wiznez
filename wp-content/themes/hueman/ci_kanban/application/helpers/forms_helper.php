<?php
function generateInputFields($fields, $inputData = array(), $forSearch = FALSE)
{
    $response = '';

    foreach($fields as $name => $field)
    {
        if($forSearch && !isset($field['searchable'])) continue;

        switch($field['type'])
        {
            case 'input':
                $response .= generate_inputField($name, $field['namespace'], $field, $inputData, $forSearch);
                break;

            case 'textarea':
                $response .= generate_textareaField($name, $field['namespace'], $field, $inputData, $forSearch);
                break;

            case 'select':
                $response .= generate_selectField($name, $field['namespace'], $field, $inputData, $forSearch);
                break;

            case 'date':
                $response .= generate_dateField($name, $field['namespace'], $field, $inputData, $forSearch);
                break;

            case 'date_monthYear':
                $response .= generate_dateField_monthYear($name, $field['namespace'], $field, $inputData, $forSearch);
                break;

            case 'password':
                $response .= generate_passwordField($name, $field['namespace'], $field); // Not searchable - duh
                break;

            case 'country':
                $response .= generate_countryField($name, $field['namespace'], $field, $inputData, $forSearch);
                break;
        }
    }
...
function generate_inputWrap($label, $input, $classes = '')
{
    $wrap  = '<div class="row input-block' . $classes . '">';
    $wrap .= '<div class="input-container small-8 columns">' . ($label ? '<label>' . $label . '</label>' : '') . $input . '</div> ';
    $wrap .= '</div>';
...
function generate_inputWrap_forSearch($label, $input, $classes = '')
{
    $wrap  = '<div class="row' . $classes . '">';
    if($label) $wrap .= '<label>' . $label . '</label>';
    $wrap .= '<div class="input-container">' . $input . '</div> ';
    $wrap .= '</div>';
...
function generate_inputField($fieldName, $namespace, $data, $inputData, $forSearch)
{
    # Namespace
    ...
    # Basic attributes
    $field .= 'class="sized_fullWidth" ';
    $field .= 'placeholder="' . (isset($data['placeholder']) ? $data['placeholder'] : $data['title']) . '" ';
    $field .= 'value="' . ($inputData && isset($inputData->$fieldName) ? $inputData->$fieldName : '') . '" ';
    $field .= 'id="' . $namespace . $fieldName . '" ';
    $field .= 'name="' . $namespace . $fieldName . '" ';
    # Conditionals
    if(isset($data['required'])) $field .= 'required ';
    ...
    return ($forSearch ? generate_inputWrap_forSearch($data['title'], $field) : generate_inputWrap($data['title'], $field));
}


function generate_passwordField($fieldName, $namespace, $data)
{
    # Namespace
    $namespace = $namespace . '_';
    ...
    return generate_inputWrap($data['title'], $field);
}


function generate_textareaField($fieldName, $namespace, $data, $inputData, $forSearch)
{
    # Namespace
    $namespace = $namespace . '_';
    ...
    # Close it
    $field .= '>' . ($inputData && isset($inputData->$fieldName) ? $inputData->$fieldName : '') . '</textarea>';

    return ($forSearch ? generate_inputWrap_forSearch($data['title'], $field) : generate_inputWrap($data['title'], $field));
}


function generate_selectField($fieldName, $namespace, $data, $inputData, $forSearch)
{
    # Namespace
    $namespace = $namespace . '_';
    ...
    if(!isset($data['defaultNull'])) $field .= '<option value="">-- Select --</option>';

    # Options
    foreach($data['options'] as $val => $title) $field .= '<option value="' . $val . '"' . ($inputData && isset($inputData->$fieldName) && $val == $inputData->$fieldName ? ' selected="selected"' : '') . '>' . $title . '</option>';
...
function generate_dateField_monthYear($fieldName, $namespace, $data, $inputData, $forSearch)
{
    # Get instance
    $CI = & get_instance();
    ...
    # Month
    $field .= '<select ';
    $field .= 'class="sized_fullWidth dateField_month" ';
    $field .= 'id="' . $namespace . $fieldName . '_month" ';
    $field .= 'name="' . $namespace . $fieldName . '_month" ';
    if(isset($data['required'])) $field .= 'required ';
    $field = rtrim($field);
    $field .= '>';
    if(!isset($data['defaultNull'])) $field .= '<option value="">-- Select --</option>';
    foreach($CI->variable->months() as $val => $title) $field .= '<option value="' . $val . '"' . ($val == $sel_month ? ' selected="selected"' : '') . '>' . $title . '</option>';
    $field .= '</select>';

    # Year
    $field .= '<select ';
    $field .= 'class="sized_fullWidth dateField_year" ';
    $field .= 'id="' . $namespace . $fieldName . '_year" ';
    $field .= 'name="' . $namespace . $fieldName . '_year" ';
    if(isset($data['required'])) $field .= 'required ';
    $field = rtrim($field);
    $field .= '>';
    if(!isset($data['defaultNull'])) $field .= '<option value="">-- Select --</option>';
    foreach($CI->variable->years() as $val => $title) $field .= '<option value="' . $val . '"' . ($val == $sel_year ? ' selected="selected"' : '') . '>' . $title . '</option>';
    $field .= '</select>';

    return generate_inputWrap($data['title'], $field, ' date_container');
}


function generate_dateField($fieldName, $namespace, $data, $inputData, $forSearch)
{
    # Get instance
    $CI = & get_instance();
    ...
    # Day
    $field .= '<select ';
    $field .= 'class="sized_fullWidth dateField_day" ';
    $field .= 'id="' . $namespace . $fieldName . '_day" ';
    $field .= 'name="' . $namespace . $fieldName . '_day" ';
    if(isset($data['required'])) $field .= 'required ';
    $field = rtrim($field);
    $field .= '>';
    if(!isset($data['defaultNull'])) $field .= '<option value="">-- Select --</option>';
    foreach($CI->variable->days() as $val => $title) $field .= '<option value="' . $val . '"' . ($val == $sel_day ? ' selected="selected"' : '') . '>' . $title . '</option>';
    $field .= '</select>';
    ...
    return generate_inputWrap($data['title'], $field, ' date_container date_container_monthYear');
}


function generate_submitButton($title, $extraStuff = '')
{
    return generate_inputWrap(FALSE, $extraStuff . '<input type="submit" class="btn" value="' . $title . '">', ' input-block-submit');
}
...
function generate_countryField($fieldName, $namespace, $data, $inputData, $forSearch)
{
    # Get instance
    $CI = & get_instance();
    ...
    # Set the initial empty
    if(!isset($data['defaultNull'])) $field .= '<option value=""></option>';

    # Options
    $field .= '<option value="Afghanistan" data-alternative-spellings="AF افغانستان">Afghanistan</option>
...
<option value="Uzbekistan" data-alternative-spellings="UZ Ўзбекистон Ozbekstan Ozbekiston">Uzbekistan</option>
...
<option value="Zimbabwe" data-alternative-spellings="ZW">Zimbabwe</option>';
    ...
    # But first add a small script
    $field .= '<script>$("#profile_country").val("' . ($CI->user->currentUserData->data_filled ? $CI->user->currentUserData->country : '') . '")</script>';

    return ($forSearch ? generate_inputWrap_forSearch($data['title'], $field) : generate_inputWrap($data['title'], $field, ' country-selector'));
}