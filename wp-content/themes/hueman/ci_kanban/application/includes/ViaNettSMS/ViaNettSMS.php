...
class ViaNettSMS
{
     ...
     */
	public function ViaNettSMS($Username, $Password,$CampaignId)
	{
		$this->Username = $Username;
		$this->Password = $Password;
        $this->CampaigId = $CampaignId;
	}
	 ...
	 */
	public function SendSMS($MsgSender, $DestinationAddress, $Message,$refno=1,$charge=0)
	{
		...
		$url = "http://smsc.vianett.no/ActiveServer/MT/?username=%s&password=%s&destinationaddr=%s&message=%s&refno=%s&campaignid=%s&pricegroup=%s&nrq=true";
		$url = sprintf($url, urlencode($this->Username), urlencode($this->Password), urlencode($DestinationAddress), urlencode($Message),$refno,$this->CampaigId,urlencode($charge));
